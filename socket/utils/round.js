...
export const startRound = async (game, io, socket) => {
  ...
  io.to(game.room).emit("game:status", {
    ...
    data: {
      ...
      cooldown: question.time || question.cooldown || 10, // use per-question timer
    },
  })

  await sleep(question.time || question.cooldown || 10)
  ...
  io.to(game.room).emit("game:status", {
    ...
    data: {
      ...
      image: question.image,
      time: question.time || question.cooldown || 10,
      totalPlayer: game.players.length,
    },
  })

  await cooldown(question.time || question.cooldown || 10, io, game.room)
  ...
  if (game.questions[game.currentQuestion + 1]) {
    ...
    // End of quiz, show leaderboard or finish
    // (existing logic will handle this)
  }
}
